...
class ESSBNetworks_Subscribe {
	...
	public static function draw_integrated_subscribe_form($salt, $popup_mode = false, $option_design = '', $is_widget = false) {
		...
		if ($user_design == 'design1') {
			...
			return self::draw_mailchimp_subscribe8($salt, $is_widget);
		}
		else {
			return self::draw_mailchimp_subscribe($salt, $is_widget);
		}
	...
	public static function draw_mailchimp_subscribe8($salt, $is_widget = false) {
		if (!function_exists('essb_subscribe_form_design8')) {
			include_once (ESSB3_PLUGIN_ROOT . 'lib/networks/essb-subscribe-design8.php');
		}
	
		return essb_subscribe_form_design8($salt, $is_widget);
	}
}